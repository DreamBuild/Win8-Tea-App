...
namespace TeaApp.Views
{
    ...
    /// 基本页，提供大多数应用程序通用的特性。
    /// </summary>
    public sealed partial class FollowTeaPage : Page
    {
        ...
        {
            TeaFollowDataSource dataSource  =new TeaFollowDataSource();
            this.DefaultViewModel["Items"] = await dataSource.GetFollowTeaItems();
        }
        ...
        {
            FollowTeaItem item = e.ClickedItem as FollowTeaItem;
            if (item != null)
            {
                ItemDetailViewModel model = new ItemDetailViewModel();
                model.Items = item.Details;
